...
void PFAlgo::setParameters(
    double nSigmaECAL, double nSigmaHCAL,
...
// PFElectrons: a new method added to set the parameters for electron
// reconstruction.
void PFAlgo::setPFEleParameters(
    double mvaEleCut, string mvaWeightFileEleID, bool usePFElectrons,
    ...
    double sumEtEcalIsoForEgammaSC_endcap, double coneEcalIsoForEgammaSC,
    double sumPtTrackIsoForEgammaSC_barrel,
    double sumPtTrackIsoForEgammaSC_endcap, unsigned int nTrackIsoForEgammaSC,
    double coneTrackIsoForEgammaSC, bool applyCrackCorrections,
    bool usePFSCEleCalib, bool useEGElectrons, bool useEGammaSupercluster) {
  ...
  pfele_ = new PFElectronAlgo(
      mvaEleCut_, mvaWeightFileEleID_, thePFSCEnergyCalibration_,
      thePFEnergyCalibration, applyCrackCorrectionsElectrons_, usePFSCEleCalib_,
      useEGElectrons_, useEGammaSupercluster_, sumEtEcalIsoForEgammaSC_barrel_,
      sumEtEcalIsoForEgammaSC_endcap_, coneEcalIsoForEgammaSC_,
      sumPtTrackIsoForEgammaSC_barrel_, sumPtTrackIsoForEgammaSC_endcap_,
      nTrackIsoForEgammaSC_, coneTrackIsoForEgammaSC_);
}

void PFAlgo::setPFPhotonParameters(
    bool usePFPhotons, std::string mvaWeightFileConvID, double mvaConvCut,
    bool useReg, std::string X0_Map,
    const std::shared_ptr<PFEnergyCalibration> &thePFEnergyCalibration,
    double sumPtTrackIsoForPhoton, double sumPtTrackIsoSlopeForPhoton) {
  ...
  pfpho_ = new PFPhotonAlgo(mvaWeightFileConvID, mvaConvCut, useReg, X0_Map,
                            *pv, thePFEnergyCalibration, sumPtTrackIsoForPhoton,
                            sumPtTrackIsoSlopeForPhoton);
  return;
}

void PFAlgo::setEGammaParameters(
    bool use_EGammaFilters, std::string ele_iso_path_mvaWeightFile,
    double ele_iso_pt, double ele_iso_mva_barrel, double ele_iso_mva_endcap,
    double ele_iso_combIso_barrel, double ele_iso_combIso_endcap,
    double ele_noniso_mva, unsigned int ele_missinghits,
    double ele_ecalDrivenHademPreselCut, double ele_maxElePtForOnlyMVAPresel,
    bool useProtectionsForJetMET,
    const edm::ParameterSet &ele_protectionsForJetMET,
    const edm::ParameterSet &ele_protectionsForBadHcal, double ph_MinEt,
    double ph_combIso, double ph_HoE, double ph_sietaieta_eb,
    double ph_sietaieta_ee, const edm::ParameterSet &ph_protectionsForJetMET,
  ...
  pfegamma_ = new PFEGammaFilters(
      ph_MinEt, ph_combIso, ph_HoE, ph_sietaieta_eb, ph_sietaieta_ee,
      ph_protectionsForJetMET, ph_protectionsForBadHcal, ele_iso_pt,
      ele_iso_mva_barrel, ele_iso_mva_endcap, ele_iso_combIso_barrel,
      ele_iso_combIso_endcap, ele_noniso_mva, ele_missinghits,
      ele_ecalDrivenHademPreselCut, ele_maxElePtForOnlyMVAPresel,
      ele_iso_path_mvaWeightFile, ele_protectionsForJetMET,
      ele_protectionsForBadHcal);

  return;
}
void PFAlgo::setEGammaCollections(
    const edm::View<reco::PFCandidate> &pfEgammaCandidates,
...
void PFAlgo::setPFPhotonRegWeights(const GBRForest *LCorrForestEB,
  ...
  pfpho_->setGBRForest(LCorrForestEB, LCorrForestEE, GCorrForestBarrel,
                       GCorrForestEndcapHr9, GCorrForestEndcapLr9,
                       PFEcalResolution);
}
...
void PFAlgo::setBadHcalTrackParams(const edm::ParameterSet &pset) {
  goodTrackDeadHcal_ptErrRel_ =
      pset.getParameter<double>("goodTrackDeadHcal_ptErrRel");
  goodTrackDeadHcal_chi2n_ =
      pset.getParameter<double>("goodTrackDeadHcal_chi2n");
  goodTrackDeadHcal_layers_ =
      pset.getParameter<uint32_t>("goodTrackDeadHcal_layers");
  goodTrackDeadHcal_validFr_ =
      pset.getParameter<double>("goodTrackDeadHcal_validFr");
  goodTrackDeadHcal_dxy_ = pset.getParameter<double>("goodTrackDeadHcal_dxy");

  goodPixelTrackDeadHcal_minEta_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_minEta");
  goodPixelTrackDeadHcal_maxPt_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_maxPt");
  goodPixelTrackDeadHcal_ptErrRel_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_ptErrRel");
  goodPixelTrackDeadHcal_chi2n_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_chi2n");
  goodPixelTrackDeadHcal_maxLost3Hit_ =
      pset.getParameter<int32_t>("goodPixelTrackDeadHcal_maxLost3Hit");
  goodPixelTrackDeadHcal_maxLost4Hit_ =
      pset.getParameter<int32_t>("goodPixelTrackDeadHcal_maxLost4Hit");
  goodPixelTrackDeadHcal_dxy_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_dxy");
  goodPixelTrackDeadHcal_dz_ =
      pset.getParameter<double>("goodPixelTrackDeadHcal_dz");
}
...
void PFAlgo::setPostHFCleaningParameters(
    bool postHFCleaning, double minHFCleaningPt, double minSignificance,
    double maxSignificance, double minSignificanceReduction,
    double maxDeltaPhiPt, double minDeltaMet) {
...
void PFAlgo::setPFVertexParameters(
    bool useVertex, const reco::VertexCollection *primaryVertices) {
...
void PFAlgo::photonAlgo(const reco::PFBlockRef &blockref, std::vector<bool>& active, std::vector<reco::PFCandidate>& tempElectronCandidates) {
 if (debug_)
    ...
    if (pfpho_->isPhotonValidCandidate(
            blockref, // passing the reference to the PFBlock
            active, // std::vector<bool> containing information about acitivity
            pfPhotonCandidates_, // pointer to candidate vector, to be filled by
                                 // the routine
            pfPhotonExtraCand,   // candidate extra vector, to be filled by the
                                 // routine
            tempElectronCandidates
            // pfElectronCandidates_   // pointer to some auziliary UNTOUCHED
            // FOR NOW
            )) {
      if (debug_)
        std::cout << " In this PFBlock we found " << pfPhotonCandidates_->size()
                  << " Photon Candidates." << std::endl;

      // CAUTION: In case we want to allow the PhotonAlgo to 'over-write' what
      // the ElectronAlgo did above
      // ........ we should NOT fill the PFCandidate-vector with the electrons
      // above (***)
...
void PFAlgo::EGammaAlgo(const reco::PFBlockRef &blockref, std::vector<bool>& active) {
    ...
    for (unsigned int ieg = 0; ieg < negmcandidates; ++ieg) {
      ...
      reco::PFCandidateRef pfEgmRef =
          pfEgammaCandidates_->refAt(ieg).castTo<reco::PFCandidateRef>();

      const PFCandidate::ElementsInBlocks &theElements =
          (*pfEgmRef).elementsInBlocks();
      PFCandidate::ElementsInBlocks::const_iterator iegfirst =
          theElements.begin();
      ...
      if (sameBlock) {
        ...
        if (egmLocalDebug)
          cout << " I am in looping on EGamma Candidates: pt "
               << (*pfEgmRef).pt() << " eta,phi " << (*pfEgmRef).eta() << ", "
        ...
        if ((*pfEgmRef).gsfTrackRef().isNonnull()) {
          ...
          if (gedEleRef.isNonnull()) {
            isGoodElectron =
                pfegamma_->passElectronSelection(*gedEleRef, *pfEgmRef, nVtx_);
            isPrimaryElectron = pfegamma_->isElectron(*gedEleRef);
            if (egmLocalDebug) {
              if (isGoodElectron)
                cout << "** Good Electron, pt " << gedEleRef->pt()
                     << " eta, phi " << gedEleRef->eta() << ", "
                     << gedEleRef->phi() << " charge " << gedEleRef->charge()
                     << " isPrimary " << isPrimaryElectron << " isoDr03 "
                     << (gedEleRef->dr03TkSumPt() +
                         gedEleRef->dr03EcalRecHitSumEt() +
                         gedEleRef->dr03HcalTowerSumEt())
                     << " mva_isolated " << gedEleRef->mva_Isolated()
                     << " mva_e_pi " << gedEleRef->mva_e_pi() << endl;
            }
          }
        }
        if ((*pfEgmRef).superClusterRef().isNonnull()) {
          ...
          if (gedPhoRef.isNonnull()) {
            ...
            if (egmLocalDebug) {
              if (isGoodPhoton)
                cout << "** Good Photon, pt " << gedPhoRef->pt() << " eta, phi "
      ...
      if (isGoodElectron) {
        ...
        if (useProtectionsForJetMET_) {
          lockTracks = true;
          isSafe = pfegamma_->isElectronSafeForJetMET(
              *gedEleRef, myPFElectron, primaryVertex_, lockTracks);
        }
        ...
        if (isSafe) {
          ...
          if (egmLocalDebug) {
            ...
            cout << " myPFElectron: pt " << myPFElectron.pt() << " eta,phi "
                 << myPFElectron.eta() << ", " << myPFElectron.phi() << " mva "
                 << myPFElectron.mva_e_pi() << " charge "
                 << myPFElectron.charge() << endl;
          }
          ...
          // The electron is considered safe for JetMET and the additional
          // tracks pointing to it are locked
          if (lockTracks) {
            const PFCandidate::ElementsInBlocks &extraTracks =
                myPFElectron.egammaExtraRef()->extraNonConvTracks();
      ...
      if (isGoodPhoton) {
        ...
        if (isSafe) {
          ...
          ::math::XYZPoint v(primaryVertex_.x(), primaryVertex_.y(),
                             primaryVertex_.z());
          myPFPhoton.setVertex(v);
          myPFPhoton.setP4(gedPhoRef->p4());
          if (egmLocalDebug) {
            cout << " PFAlgo: found a photon with NEW EGamma code " << endl;
            cout << " myPFPhoton: pt " << myPFPhoton.pt() << " eta,phi "
                 << myPFPhoton.eta() << ", " << myPFPhoton.phi() << " charge "
                 << myPFPhoton.charge() << endl;
          }
...
bool PFAlgo::recoTracksNotHCAL(const reco::PFBlock &block, reco::PFBlock::LinkData& linkData, const edm::OwnVector<reco::PFBlockElement> &elements, const reco::PFBlockRef &blockref, std::vector<bool>& active, bool goodTrackDeadHcal, bool hasDeadHcal, unsigned int iTrack, std::multimap<double, unsigned>& ecalElems, reco::TrackRef& trackRef) {
  if (debug_)
    std::cout << "Now deals with tracks linked to no HCAL clusters. Was "
                 "HCal active? "
              << (!hasDeadHcal) << std::endl;
  ...
  if (!thisIsAMuon && elements[iTrack].trackRef()->ptError() > ptError_) {
    ...
    double resol = neutralHadronEnergyResolution(
        trackMomentum, elements[iTrack].trackRef()->eta());
    resol *= trackMomentum;
    ...
    if (!ecalElems.empty()) {
      ...
      if (hasDeadHcal) {
        ...
        block.associatedElements(thisEcal, linkData, sortedTracks,
      ...
      if (useCluster) {
        ...
        resol = neutralHadronEnergyResolution(
            trackMomentum, clusterRef->positionREP().Eta());
        resol *= trackMomentum;
      }
    }
    ...
    if (deficit > nSigmaTRACK_ * resol && !isPrimary &&
      ...
      if (debug_)
        std::cout << elements[iTrack] << std::endl
                  << " deficit " << deficit << " > " << nSigmaTRACK_
                  << " x " << resol << " track pt " << trackRef->pt()
                  ...
                  << std::abs(trackRef->dxy(primaryVertex_.position()))
                  << " +- " << trackRef->dxyError() << " |dz| "
                  << std::abs(trackRef->dz(primaryVertex_.position()))
                  << " +- " << trackRef->dzError()
  ...
  // Some cleaning : secondary tracks without calo's and large momentum must
  // be fake
  double Dpt = trackRef->ptError();

  if (rejectTracks_Step45_ && ecalElems.empty() && trackMomentum > 30. &&
      Dpt > 0.5 &&
    ...
    if (isPrimaryOrSecondary && dptRel < dptRel_DispVtx_)
      return true;
    unsigned nHits = elements[iTrack]
                         .trackRef()
                         ->hitPattern()
                         .trackerLayersWithMeasurement();
    unsigned int NLostHit =
        trackRef->hitPattern().trackerLayersWithoutMeasurement(
            HitPattern::TRACK_HITS);

    if (debug_)
      std::cout << "A track (algo = " << trackRef->algo()
                ...
                << " (" << NLostHit << ") hits (lost hits) has been cleaned"
                << std::endl;
    active[iTrack] = false;
    return true;
  }
  ...
  if (thisIsAMuon) {
    (*pfCandidates_)[tmpi[0]].setEcalEnergy(
        clusterRef->energy(), std::min(clusterRef->energy(), muonECAL_[0]));
    (*pfCandidates_)[tmpi[0]].setHcalEnergy(0., 0.);
    (*pfCandidates_)[tmpi[0]].setHoEnergy(0., 0.);
    (*pfCandidates_)[tmpi[0]].setPs1Energy(0);
    (*pfCandidates_)[tmpi[0]].setPs2Energy(0);
  ...
  block.associatedElements(thisEcal, linkData, sortedTracks,
                           ...
                           reco::PFBlock::LINKTEST_ALL);
  if (debug_)
    cout << "the closest ECAL cluster, id " << thisEcal << ", has "
         << sortedTracks.size()
         << " associated tracks, now processing them. " << endl;

  if (hasDeadHcal && !sortedTracks.empty()) {
    // GP: only allow the ecal cluster closest to this track to be
    // considered
    if (debug_)
      cout << " the closest track to ECAL " << thisEcal << " is "
           << sortedTracks.begin()->second << " (distance "
           << sortedTracks.begin()->first << ")" << endl;
    if (sortedTracks.begin()->second != iTrack) {
      if (debug_)
        cout << " the closest track to ECAL " << thisEcal << " is "
             << sortedTracks.begin()->second
             << " which is not the one being processed. Will skip ECAL "
                "linking for this track"
             << endl;
      (*pfCandidates_)[tmpi[0]].setEcalEnergy(0., 0.);
      (*pfCandidates_)[tmpi[0]].setHcalEnergy(0., 0.);
      (*pfCandidates_)[tmpi[0]].setHoEnergy(0., 0.);
      ...
      if (debug_)
        cout << " the closest track to ECAL " << thisEcal << " is "
             << sortedTracks.begin()->second
             << " which is the one being processed. Will skip ECAL linking "
                "for all other track"
             << endl;
      sortedTracks.clear();
    }
  ...
  for (auto const &trk : sortedTracks) {
    ...
    block.associatedElements(jTrack, linkData, sortedECAL,
    ...
    if (debug_) {
      cout << " found track " << jTrack
           << (thatIsAMuon ? " (muon) " : " (non-muon)")
    ...
    if (!thatIsAMuon && trackRef->ptError() > ptError_) {
      ...
      double resol = nSigmaTRACK_ * neutralHadronEnergyResolution(
    ...
    if (!thatIsAMuon) {
      if (debug_)
        std::cout << "Track momentum increased from " << trackMomentum
                  << " GeV ";
      trackMomentum += trackRef->p();
      if (debug_) {
    ...
    if (thatIsAMuon) {
      (*pfCandidates_)[tmpi.back()].setEcalEnergy(
          clusterRef->energy(),
      ...
      (*pfCandidates_)[tmpi.back()].addElementInBlock(blockref,
                                                      kTrack.back());
    }
  }
  ...
  for (auto const &ecal : ecalElems) {
    ...
    const bool skip =
        std::none_of(kTrack.begin(), kTrack.end(), [&](unsigned iTrack) {
    ...
    // std::cout << "Warning - first ECAL cluster at a distance of " << dist
    // << "from the closest track!" << std::endl; cout<<"\telement
    // "<<elements[index]<<" linked with distance = "<< dist <<endl;
    // cout<<"\telement "<<elements[iTrack]<<" linked with distance = "<<
    // dist <<endl;
    //}

    // Check the presence of preshower clusters in the vicinity
    // Preshower cluster closer to another ECAL cluster are ignored.
    // JOSH: This should use the association map already produced by the
    // cluster corrector for consistency, but should be ok for now
    vector<double> ps1Ene{0.};
    associatePSClusters(index, reco::PFBlockElement::PS1, block, elements,
                        linkData, active, ps1Ene);
    vector<double> ps2Ene{0.};
    associatePSClusters(index, reco::PFBlockElement::PS2, block, elements,
                        linkData, active, ps2Ene);

    // KH: use raw ECAL energy for PF hadron calibration. use calibrated
    // ECAL energy when adding PF photons
    const double ecalEnergy = clusterRef->energy();
    const double ecalEnergyCalibrated =
        clusterRef->correctedEnergy(); // calibrated based on the egamma
                                       // hypothesis
    if (debug_)
      std::cout << "Corrected ECAL(+PS) energy = " << ecalEnergy
                << std::endl;

    // Since the electrons were found beforehand, this track must be a
    // hadron. Calibrate the energy under the hadron hypothesis.
    ...
    if (debug_)
      std::cout << "The total calibrated energy so far amounts to = "
                << calibEcal << " (slope = " << slopeEcal << ")"
                << std::endl;
    ...
    if (connectedToEcal && calibEcal - trackMomentum >= 0.) {
      ...
      block.associatedElements(index, linkData, assTracks,
                               reco::PFBlockElement::TRACK,
                               reco::PFBlock::LINKTEST_ALL);

      auto &ecalCand = (*pfCandidates_)[reconstructCluster(
          *clusterRef, ecalEnergyCalibrated)]; // KH: use the PF ECAL
                                               // cluster calibrated energy
      ...
      if (!assTracks.empty()) {
        ...
        const ::math::XYZPointF &chargedPosition =
            dynamic_cast<const reco::PFBlockElementTrack *>(
  ...
  if (connectedToEcal) {
    ...
    for(IE ie = sortedTracks.begin(); ie != sortedTracks.end() &&
    neutralEnergy > 0; ++ie ) { unsigned jTrack = ie->second;
      // The loop is on the other tracks !
    ...
    */
    ...
    double resol = neutralHadronEnergyResolution(
        trackMomentum, pivotalRef->positionREP().Eta());
    resol *= trackMomentum;
    if (neutralEnergy > std::max(0.5, nSigmaECAL_ * resol)) {
      ...
      (*pfCandidates_)[tmpj].setEcalEnergy(pivotalRef->energy(),
                                           neutralEnergy);
    ...
    for (unsigned ic = 0; ic < tmpi.size(); ++ic) {
      ...
      double ecalCal =
          bNeutralProduced
              ? (calibEcal - neutralEnergy * slopeEcal) * fraction
              : calibEcal * fraction;
      double ecalRaw = totalEcal * fraction;

      if (debug_)
        cout << "The fraction after photon supression is " << fraction
  ...
  // Fill the element_in_block for tracks that are eventually linked to no
  // ECAL clusters at all.
  for (unsigned ic = 0; ic < tmpi.size(); ++ic) {
    const PFCandidate &pfc = (*pfCandidates_)[tmpi[ic]];
    const PFCandidate::ElementsInBlocks &eleInBlocks =
        pfc.elementsInBlocks();
...
void PFAlgo::EleLoop1(const reco::PFBlock &block, reco::PFBlock::LinkData& linkData, const edm::OwnVector<reco::PFBlockElement> &elements, std::vector<bool>& active,
  const reco::PFBlockRef &blockref, ElementIndices& inds, std::vector<bool> &deadArea) {
    ...
    if (debug_ && muonRef.isNonnull()) {
      cout << "track " << iTrack << " has a valid muon reference. "
           << std::endl;
      cout << "   - isMuon: " << PFMuonAlgo::isMuon(muonRef) << endl;
      cout << "   - isGlobalTightMuon: "
           << PFMuonAlgo::isGlobalTightMuon(muonRef) << endl;
      cout << "   - isTrackerTightMuon: "
           << PFMuonAlgo::isTrackerTightMuon(muonRef) << endl;
      cout << "   - isIsolatedMuon: " << PFMuonAlgo::isIsolatedMuon(muonRef)
           << endl;
    }
    ...
    if (active[iTrack] && isFromSecInt(elements[iEle], "primary")) {
      bool isPrimaryTrack = elements[iEle]
                                .displacedVertexRef(PFBlockElement::T_TO_DISP)
                                ->displacedVertexRef()
    ...
    if (debug_) {
      cout << "PFAlgo:processBlock "
           ...
           << inds.hcalIs.size() << " " << inds.hoIs.size() << endl;
    }
    ...
    block.associatedElements(iTrack, linkData, ecalElems,
                             reco::PFBlockElement::ECAL,
                             reco::PFBlock::LINKTEST_ALL);

    std::multimap<double, unsigned> hcalElems;
    block.associatedElements(iTrack, linkData, hcalElems,
                             reco::PFBlockElement::HCAL,
                             reco::PFBlock::LINKTEST_ALL);

    if (debug_) {
      std::cout << "\tTrack " << iTrack << " is linked to " << ecalElems.size()
                << " ecal and " << hcalElems.size() << " hcal elements"
                << std::endl;
      for (const auto &pair : ecalElems) {
        std::cout << "ecal: dist " << pair.first << "\t elem " << pair.second
                  << std::endl;
      }
    ...
    // for (auto it = hcalElems.begin(), ed = hcalElems.end(); it != ed; /*NOTE
    // NO ++it HERE */ ) {
    //    if (deadArea[it->second]) { hasDeadHcal = true; it =
    //    hcalElems.erase(it); } // std::multimap::erase returns iterator to
    //    next else ++it;
    //}
    //--- option (2) --
    bool hasDeadHcal = false;
    ...
    // for (auto it = hcalElems.begin(), ed = hcalElems.end(); it != ed; /*NOTE
    // NO ++it HERE */ ) {
    //    if (deadArea[it->second]) { it = hcalElems.erase(it); } //
    //    std::multimap::erase returns iterator to next else { hasDeadHcal =
    //    false; }
    //}
    ...
    if (hasDeadHcal) {
      goodTrackDeadHcal =
          (trackRef->ptError() < goodTrackDeadHcal_ptErrRel_ * trackRef->pt() &&
      ...
      if (!goodTrackDeadHcal &&
          std::abs(trackRef->eta()) >
              goodPixelTrackDeadHcal_minEta_ && // high eta
          trackRef->hitPattern().pixelLayersWithMeasurement() >=
              3 && // pixel track
          trackRef->hitPattern().trackerLayersWithoutMeasurement(
              HitPattern::TRACK_HITS) == 0 &&
          trackRef->hitPattern().trackerLayersWithoutMeasurement(
              HitPattern::MISSING_INNER_HITS) == 0 &&
          trackRef->hitPattern().trackerLayersWithoutMeasurement(
              HitPattern::MISSING_OUTER_HITS) <=
          ...
          trackRef->normalizedChi2() <
              goodPixelTrackDeadHcal_chi2n_ && // tighter cut
          std::abs(trackRef->dxy(primaryVertex_.position())) <
              goodPixelTrackDeadHcal_dxy_ &&
          std::abs(trackRef->dz(primaryVertex_.position())) <
              goodPixelTrackDeadHcal_dz_ &&
          trackRef->ptError() <
              goodPixelTrackDeadHcal_ptErrRel_ * trackRef->pt() && // sanity
          trackRef->pt() < goodPixelTrackDeadHcal_maxPt_) {        // sanity
        goodTrackDeadHcal = true;
        // FIXME: may decide to do something to the track pT
      }
      // if (!goodTrackDeadHcal &&
      // trackRef->hitPattern().trackerLayersWithMeasurement() == 4 &&
      // trackRef->validFraction() == 1
      if (debug_)
        cout << " track pt " << trackRef->pt() << " +- " << trackRef->ptError()
             << " layers valid "
             << trackRef->hitPattern().trackerLayersWithMeasurement()
             << ", lost "
             << trackRef->hitPattern().trackerLayersWithoutMeasurement(
             ...
             << std::abs(trackRef->dz(primaryVertex_.position())) << " +- "
             << trackRef->dzError()
             << (goodTrackDeadHcal ? " passes " : " fails ") << "quality cuts"
             << std::endl;
    }

    // When a track has no HCAL cluster linked, but another track is linked to
    // the same ECAL cluster and an HCAL cluster, link the track to the HCAL
    // cluster for later analysis
    if (hcalElems.empty() && !ecalElems.empty() && !hasDeadHcal) {
      ...
      block.associatedElements(index, linkData, sortedTracks,
                               ...
                               reco::PFBlock::LINKTEST_ALL);
      if (debug_)
        std::cout << "The closest ECAL cluster is linked to "
                  << sortedTracks.size()
      ...
      for (auto const &trk : sortedTracks) {
        ...
        // Check if this track is also linked to an HCAL
        std::multimap<double, unsigned> sortedHCAL;
        block.associatedElements(jTrack, linkData, sortedHCAL,
                                 reco::PFBlockElement::HCAL,
                                 reco::PFBlock::LINKTEST_ALL);
        if (sortedHCAL.empty())
          continue;
        if (debug_)
          std::cout << "  and with an HCAL cluster "
                    << sortedHCAL.begin()->second << std::endl;
        ntt++;

        // In that case establish a link with the first track
        block.setLink(iTrack, sortedHCAL.begin()->second,
                      sortedECAL.begin()->first, linkData,
    ...
    if (usePFElectrons_ == false) {
      block.associatedElements(iTrack, linkData, gsfElems,
    ...
    for (auto const &ecal : ecalElems) {
      ...
      // Flag ECAL clusters for which the corresponding track is not linked to
      // an HCAL cluster
    ...
    if (hcalElems.empty()) {
      auto ret = recoTracksNotHCAL(block, linkData, elements, blockref, active, goodTrackDeadHcal, hasDeadHcal, iTrack, ecalElems, trackRef);
      if (ret) {
        continue;
      }
    ...
    for (auto const &hcal : hcalElems) {
      ...
      if (debug_) {
        double dist =
            block.dist(iTrack, index, linkData, reco::PFBlock::LINKTEST_ALL);
        cout << "\telement " << elements[index] << " linked with distance "
             << dist << endl;
      }
...
void PFAlgo::HCALLoop(const reco::PFBlock &block, reco::PFBlock::LinkData& linkData, const edm::OwnVector<reco::PFBlockElement> &elements, std::vector<bool>& active, const reco::PFBlockRef &blockref, ElementIndices& inds, std::vector<bool> &deadArea) {
 for (unsigned iHcal : inds.hcalIs) {
    ...
    // associated tracks
    std::multimap<double, unsigned> sortedTracks;
    block.associatedElements(iHcal, linkData, sortedTracks,
                             reco::PFBlockElement::TRACK,
                             reco::PFBlock::LINKTEST_ALL);
    ...
    std::multimap<double, std::tuple<unsigned, ::math::XYZVector, double>>
        ecalSatellites; // last element (double) : correction under the egamma
                        // hypothesis
    std::tuple<unsigned, ::math::XYZVector, double> fakeSatellite(
        iHcal, ::math::XYZVector(0., 0., 0.), 1.);
    ...
    // Keep track of how much energy is assigned to calorimeter-vs-track
    // energy/momentum excess
    ...
    std::vector<std::tuple<unsigned, ::math::XYZVector, double>>
        ecalClusters; // last element (double) : correction under the egamma
                      // hypothesis
    ...
    for (auto const &trk : sortedTracks) {
      ...
      // Check the track has not already been used (e.g., in electrons,
      // conversions...)
      if (!active[iTrack])
        continue;
      // Sanity check 1
      ...
      const ::math::XYZPointF &chargedPosition =
          dynamic_cast<const reco::PFBlockElementTrack *>(&elements[iTrack])
              ->positionAtECALEntrance();
      ::math::XYZVector chargedDirection(
          chargedPosition.X(), chargedPosition.Y(), chargedPosition.Z());
      ...
      block.associatedElements(iTrack, linkData, sortedEcals,
                               reco::PFBlockElement::ECAL,
                               reco::PFBlock::LINKTEST_ALL);

      if (debug_)
        cout << "\t\t\tnumber of Ecal elements linked to this track: "
             << sortedEcals.size() << endl;

      // look for HO elements associated to iTrack (associated to iHcal)
      std::multimap<double, unsigned> sortedHOs;
      if (useHO_) {
        block.associatedElements(iTrack, linkData, sortedHOs,
      ...
      if (thisIsAMuon) {
        if (debug_) {
          std::cout << "\t\tThis track is identified as a muon - remove it "
                       "from the stack"
                    << std::endl;
          std::cout << "\t\t" << elements[iTrack] << std::endl;
        }
        ...
        // if muon is isolated and muon momentum exceeds the calo energy, absorb
        // the calo energy rationale : there has been a EM showering by the muon
        // in the calorimeter - or the coil - and we don't want to double count
        // the energy
        ...
        if (thisIsAnIsolatedMuon) {
          ...
          // std::cout << "muon p / total calo = " << muonRef->p() << " "  <<
          // (pfCandidates_->back()).p() << " " << totalCaloEnergy << std::endl;
        ...
        if (!sortedEcals.empty()) {
          ...
          muonEcal =
              std::min(muonECAL_[0] + muonECAL_[1], eclusterref->energy());
          if (letMuonEatCaloEnergy)
            muonEcal = eclusterref->energy();
          // If the muon expected energy accounts for the whole ecal cluster
          // energy, lock the ecal cluster
        ...
        if (useHO_) {
          if (!sortedHOs.empty()) {
            ...
            // If the muon expected energy accounts for the whole HO cluster
            // energy, lock the HO cluster
      ...
      double Dpt = trackRef->ptError();
      double blowError =
          PFTrackAlgoTools::errorScale(trackRef->algo(), factors45_);
      ...
      // Also keep the total track momentum error for comparison with the calo
      // energy
      double Dp = trackRef->qoverpError() * trackMomentum * trackMomentum;
      sumpError2 += Dp * Dp;

      bool connectedToEcal = false; // Will become true if there is at least one
                                    // ECAL cluster connected
      if (!sortedEcals.empty()) {   // start case: at least one ecal element
                                    // associated to iTrack
        ...
        for (auto const &ecal : sortedEcals) {
          ...
          // Check if this ECAL is not closer to another track - ignore it in
          // that case
          std::multimap<double, unsigned> sortedTracksEcal;
          block.associatedElements(iEcal, linkData, sortedTracksEcal,
                                   reco::PFBlockElement::TRACK,
                                   reco::PFBlock::LINKTEST_ALL);
          ...
          //                              reco::PFBlock::LINKTEST_ALL);
          double distEcal =
              block.dist(jTrack, iEcal, linkData, reco::PFBlock::LINKTEST_ALL);

          // totalEcal += eclusterref->energy();
          // float ecalEnergyUnCalibrated = eclusterref->energy();
          // std::cout << "Ecal Uncalibrated " << ecalEnergyUnCalibrated <<
          // std::endl;

          float ecalEnergyCalibrated =
              eclusterref->correctedEnergy(); // calibrated based on the egamma
                                              // hypothesis
          ...
          if (!connectedToEcal) { // This is the closest ECAL cluster - will add
                                  // its energy later
            ...
            connectedToEcal = true;
            // PJ 1st-April-09 : To be done somewhere !!! (Had to comment it,
            // but it is needed) currentChargedHadron.addElementInBlock(
            // blockref, iEcal );

            // KH: we don't know if this satellite is due to egamma or hadron
            // shower. use raw energy for PF hadron calibration. store also
            // calibration constant.
            double ecalCalibFactor =
                (ecalEnergy > 1E-9) ? ecalEnergyCalibrated / ecalEnergy : 1.;
      ...
      if (useHO_ && !sortedHOs.empty()) { // start case: at least one ho element
                                          // associated to iTrack
        ...
        for (auto const &ho : sortedHOs) {
          ...
          // Check if this HO is not closer to another track - ignore it in that
          // case
          std::multimap<double, unsigned> sortedTracksHO;
          block.associatedElements(iHO, linkData, sortedTracksHO,
                                   reco::PFBlockElement::TRACK,
                                   reco::PFBlock::LINKTEST_ALL);
    ...
    // Include totalHO in totalHCAL for the time being (it will be calibrated as
    // HCAL energy)
    totalHcal += totalHO;
    ...
    double Caloresolution = neutralHadronEnergyResolution(
        totalChargedMomentum, hclusterref->positionREP().Eta());
    Caloresolution *= totalChargedMomentum;
    // Account for muons
    Caloresolution = std::sqrt(Caloresolution * Caloresolution + muonHCALError +
                               muonECALError);
    ...
    // Loop over all ECAL satellites, starting for the closest to the various
    // tracks and adding other satellites until saturation of the total track
    // momentum Note : for code simplicity, the first element of the loop is the
    // HCAL cluster with 0 energy in the ECAL
    for (auto const &ecalSatellite : ecalSatellites) {
      ...
      //
      totalEcal += sqrt(std::get<1>(ecalSatellite.second)
                            .Mag2()); // KH: raw ECAL energy for input to PF
                                      // hadron calibration
      totalEcalEGMCalib +=
          sqrt(std::get<1>(ecalSatellite.second).Mag2()) *
          std::get<2>(ecalSatellite.second); // KH: calibrated ECAL energy under
                                             // the egamma hypothesis
      photonAtECAL +=
          std::get<1>(ecalSatellite.second) *
          std::get<2>(ecalSatellite.second); // KH: calibrated ECAL energy under
                                             // the egamma hypothesis
      calibEcal =
          std::max(0., totalEcal); // KH: preparing for hadron calibration
      ...
      // Continue looping until all closest clusters are exhausted and as long
      // as the calorimetric energy does not saturate the total momentum.
      if (ecalSatellite.first < 0. || caloEnergy - totalChargedMomentum <= 0.) {
        if (debug_)
          cout << "\t\t\tactive, adding " << std::get<1>(ecalSatellite.second)
               << " to ECAL energy, and locking" << endl;
        active[std::get<0>(ecalSatellite.second)] = false;
        double clusterEnergy =
            sqrt(std::get<1>(ecalSatellite.second).Mag2()) *
            std::get<2>(ecalSatellite.second); // KH: ECAL energy calibrated
                                               // under the egamma hypothesis
        if (clusterEnergy >
            50) { // KH: used to split energetic ecal clusters (E>50 GeV)
      ...
      totalEcalEGMCalib -= sqrt(std::get<1>(ecalSatellite.second).Mag2()) *
                           std::get<2>(ecalSatellite.second);
      photonAtECAL -=
          std::get<1>(ecalSatellite.second) * std::get<2>(ecalSatellite.second);
      calibEcal = previousCalibEcal;
    ...
    // double Caloresolution = neutralHadronEnergyResolution(
    // totalChargedMomentum, hclusterref->positionREP().Eta()); Caloresolution *=
    ...
    if (totalChargedMomentum - caloEnergy > nSigmaTRACK_ * Caloresolution) {
      ...
      cout<<"\t\tsum p    = "<<totalChargedMomentum<<" +-
      "<<sqrt(sumpError2)<<endl; cout<<"\t\tsum ecal = "<<totalEcal<<endl;
      ...
      cout<<endl;
      cout << "\t\tNumber/momentum of muons found in the block : " << nMuons <<
      std::endl;
      */
      ...
      if (nMuons > 0) {
        for (auto const &trk : associatedTracks) {
          ...
          auto &muon =
              (*pfCandidates_)[reconstructTrack(elements[iTrack], true)];

          muon.addElementInBlock(blockref, iTrack);
          muon.addElementInBlock(blockref, iHcal);
          const double muonHcal =
              std::min(muonHCAL_[0] + muonHCAL_[1], totalHcal - totalHO);
          double muonHO = 0.;
          ...
          if (totalEcal > 0.)
            calibEcal -=
                std::min(calibEcal, muonECAL_[0] * calibEcal / totalEcal);
          if (totalHcal > 0.)
            calibHcal -=
                std::min(calibHcal, muonHCAL_[0] * calibHcal / totalHcal);
          ...
          // Commented out: Keep looking for muons since they often come in
          // pairs -Matt if ( totalChargedMomentum < caloEnergy ) break;
        }
        // New calo resolution.
        Caloresolution = neutralHadronEnergyResolution(
            totalChargedMomentum, hclusterref->positionREP().Eta());
        Caloresolution *= totalChargedMomentum;
        Caloresolution = std::sqrt(Caloresolution * Caloresolution +
                                   muonHCALError + muonECALError);
      }
    }
    ...
    if (debug_) {
      ...
      cout << "\t\tsum hcal = " << totalHcal << endl;
      cout << "\t\t => Calo Energy = " << caloEnergy << " +- " << Caloresolution
           << endl;
      cout << "\t\t => Calo Energy- total charged momentum = "
           << caloEnergy - totalChargedMomentum << " +- "
    ...
    if (rejectTracks_Bad_ &&
      ...
      for (auto const &trk : associatedTracks) {
        ...
        if (fabs(trk.first) < ptError_)
          break;
        // What would become the block charged momentum if this track were
        // removed
        const double wouldBeTotalChargedMomentum =
        ...
        if (wouldBeTotalChargedMomentum > caloEnergy) {
          if (debug_ && isSecondary) {
            cout << "In bad track rejection step dptRel = " << dptRel
                 << " dptRel_DispVtx_ = " << dptRel_DispVtx_ << endl;
            cout << "The calo energy would be still smaller even without this "
                    "track but it is attached to a NI"
                 << endl;
          }
          ...
          if (debug_)
            std::cout << "\tElement  " << elements[iTrack]
                      ...
                      << std::endl;
          // Just rescale the nth worst track momentum to equalize the calo
          // energy
        } else {
          if (isPrimary)
            break;
          corrTrack = iTrack;
          corrFact = (caloEnergy - wouldBeTotalChargedMomentum) /
                     elements[trk.second.first].trackRef()->p();
          ...
          if (debug_)
            std::cout << "\tElement  " << elements[iTrack]
                      ...
                      << " Now the total charged momentum is "
    ...
    // New determination of the calo and track resolution avec track
    // deletion/rescaling.
    Caloresolution = neutralHadronEnergyResolution(
        totalChargedMomentum, hclusterref->positionREP().Eta());
    Caloresolution *= totalChargedMomentum;
    ...
    // Check if the charged momentum is still very inconsistent with the calo
    // measurement. In this case, just drop all tracks from 4th and 5th
    // iteration linked to this block
    ...
    if (abs(totalChargedMomentum - caloEnergy) < nsigma * TotalError) {
      ...
      if (debug_) {
        cout << "\t\tcase 1: COMPATIBLE "
             ...
             << abs(caloEnergy - totalChargedMomentum) << " < " << nsigma
             << " x " << TotalError << endl;
        if (maxDPovP < 0.1)
    ...
    /////////////// NEUTRAL DETECTION  ////////////////
      ...
      /*
      //If it's isolated don't create neutrals since the energy deposit is
      always coming from a showering muon bool thisIsAnIsolatedMuon = false;
      for(IE ie = sortedTracks.begin(); ie != sortedTracks.end(); ++ie ) {
        unsigned iTrack = ie->second;
        if(PFMuonAlgo::isIsolatedMuon(elements[iTrack].muonRef()))
      thisIsAnIsolatedMuon = true;
      }
      if(thisIsAnIsolatedMuon){
        if(debug_)cout<<" Not looking for neutral b/c this is an isolated muon
      "<<endl; break;
      }
      */
      ...
      double ePhoton =
          (caloEnergy - totalChargedMomentum) /
          slopeEcal; // KH: this slopeEcal is computed based on ECAL energy
                     // under the hadron hypothesis, thought we are creating
                     // photons. This is a fuzzy case, but it should be better
                     // than corrected twice under both egamma and hadron
                     // hypotheses.

      if (debug_) {
        if (!sortedTracks.empty()) {
          cout << "\t\tcase 2: NEUTRAL DETECTION " << caloEnergy << " > "
               << nsigma << "x" << TotalError << " + " << totalChargedMomentum
               << endl;
      ...
      std::vector<double> particleEnergy, ecalEnergy, hcalEnergy, rawecalEnergy,
          rawhcalEnergy;
      ...
      if (ePhoton < totalEcal || eNeutralHadron - calibEcal < 1E-10) {
        ...
        if (!maxEcalRef.isNull()) {
          // So the merged photon energy is,
          mergedPhotonEnergy =
              totalEcalEGMCalib; // KH: use calibrated ECAL energy under the
                                 // egamma hypothesis
        }
        // ... and assign the remaining excess to neutral hadrons using the
        // direction of ecal clusters
        mergedNeutralHadronEnergy = eNeutralHadron - calibEcal;
      }

      if (mergedPhotonEnergy > 0) {
        // Split merged photon into photons for each energetic ecal cluster
        // (necessary for jet substructure reconstruction) make only one merged
        // photon if less than 2 ecal clusters KH: this part still needs review,
        // after using non-corrected ECAL energy for PF hadron calibrations
        if (ecalClusters.size() <= 1) {
          ecalClusters.clear();
          ecalClusters.emplace_back(
              maxiEcal, photonAtECAL,
              1.); // KH: calibration factor of 1, which should be ok as long as
                   // sumEcalClusters is consistent with photonAtECAL in this
                   // case
        ...
        for (auto const &pae : ecalClusters) {
          const double clusterEnergyCalibrated =
              ...
              std::get<2>(pae); // KH: calibrated under the egamma hypothesis.
                                // Note: sumEcalClusters is normally calibrated
                                // under egamma hypothesis
          particleEnergy.push_back(mergedPhotonEnergy *
                                   clusterEnergyCalibrated / sumEcalClusters);
          particleDirection.push_back(std::get<1>(pae));
          ecalEnergy.push_back(mergedPhotonEnergy * clusterEnergyCalibrated /
                               sumEcalClusters);
      ...
      if (mergedNeutralHadronEnergy > 1.0) {
        // Split merged neutral hadrons according to directions of energetic
        // ecal clusters (necessary for jet substructure reconstruction) make
        // only one merged neutral hadron if less than 2 ecal clusters
        if (ecalClusters.size() <= 1) {
          ecalClusters.clear();
          ecalClusters.emplace_back(
              iHcal, hadronAtECAL,
              1.); // KH: calibration factor of 1, which should be ok as long as
                   // sumEcalClusters is consistent with photonAtECAL
      ...
      for (unsigned iPivot = 0; iPivot < iPivotal.size(); ++iPivot) {
        ...
        if (particleEnergy[iPivot] < 0.)
          std::cout << "ALARM = Negative energy ! " << particleEnergy[iPivot]
                    << std::endl;

        const bool useDirection = true;
        auto &neutral = (*pfCandidates_)[reconstructCluster(
            *pivotalClusterRef[iPivot], particleEnergy[iPivot], useDirection,
            particleDirection[iPivot].X(), particleDirection[iPivot].Y(),
        ...
        if (!useHO_) {
          ...
          neutral.setHcalEnergy(max(rawhcalEnergy[iPivot] - totalHO, 0.0),
                                hcalEnergy[iPivot] *
                                    (1. - totalHO / rawhcalEnergy[iPivot]));
          neutral.setHoEnergy(totalHO, totalHO * hcalEnergy[iPivot] /
                                           rawhcalEnergy[iPivot]);
        }
    ...
    // JB: The question is: we've resolved the merged photons cleanly, but how
    // should the remaining hadrons be assigned the remaining ecal energy?
    ...
    /*
     if(totalEcal>0) {
       ...
       // totalHcalEnergyCalibrated -=
     calibration_->paramECALplusHCAL_slopeECAL() * totalEcal;
     }
     */
    ...
    for (unsigned index : chargedHadronsIndices) {
      ...
      if (!useHO_) {
        chargedHadron.setHcalEnergy(fraction * totalHcal,
                                    fraction * totalHcalEnergyCalibrated);
        chargedHadron.setHoEnergy(0., 0.);
      } else {
        chargedHadron.setHcalEnergy(fraction * max(totalHcal - totalHO, 0.0),
                                    fraction * totalHcalEnergyCalibrated *
                                        (1. - totalHO / totalHcal));
        chargedHadron.setHoEnergy(fraction * totalHO,
                                  fraction * totalHO *
                                      totalHcalEnergyCalibrated / totalHcal);
      }
      // JB: fixing up (previously omitted) setting of ECAL energy gouzevit
      chargedHadron.setEcalEnergy(fraction * totalEcal,
                                  fraction * totalEcalEnergyCalibrated);
    }
    ...
    for (auto const &ecalSatellite : ecalSatellites) {
      ...
      // Find the associated tracks
      std::multimap<double, unsigned> assTracks;
      block.associatedElements(iEcal, linkData, assTracks,
                               reco::PFBlockElement::TRACK,
                               reco::PFBlock::LINKTEST_ALL);
      ...
      double ecalClusterEnergyCalibrated =
          ...
          std::get<2>(
              ecalSatellite
                  .second); // KH: calibrated under the egamma hypothesis
                            // (rawEcalClusterEnergy * calibration)
      ...
      if (fabs(eclusterref->energy() -
               sqrt(std::get<1>(ecalSatellite.second).Mag2())) > 1e-3 ||
          fabs(eclusterref->correctedEnergy() - ecalClusterEnergyCalibrated) >
              1e-3)
        edm::LogWarning("PFAlgo:processBlock")
            << "ecalCluster vs ecalSatellites look inconsistent (eCluster E, "
               "calibE, ecalSatellite E, calib E): "
...
void PFAlgo::HCALUnlinkedLoop(const reco::PFBlock &block, reco::PFBlock::LinkData& linkData, const edm::OwnVector<reco::PFBlockElement> &elements, std::vector<bool>& active, const reco::PFBlockRef &blockref, ElementIndices& inds, std::vector<bool> &deadArea) {
  for (unsigned iHcal : inds.hcalIs) {
    ...
    std::multimap<double, unsigned> ecalElems;
    block.associatedElements(iHcal, linkData, ecalElems,
                             reco::PFBlockElement::ECAL,
                             reco::PFBlock::LINKTEST_ALL);
    ...
    for (auto const &ecal : ecalElems) {
      ...
      std::multimap<double, unsigned> hcalElems;
      block.associatedElements(iEcal, linkData, hcalElems,
                               reco::PFBlockElement::HCAL,
                               reco::PFBlock::LINKTEST_ALL);

      const bool isClosest = std::none_of(
          hcalElems.begin(), hcalElems.end(), [&](auto const &hcal) {
      ...
      if (debug_) {
        cout << "\telement " << elements[iEcal] << " linked with dist " << dist
             << endl;
      ...
      double ecalEnergy =
          eclusterRef->energy(); // ecalEnergy = eclusterRef->correctedEnergy();
    ...
    if (useHO_) {
      ...
      block.associatedElements(iHcal, linkData, hoElems,
                               reco::PFBlockElement::HO,
                               reco::PFBlock::LINKTEST_ALL);
      ...
      for (auto const &ho : hoElems) {
        ...
        // Check if not closer from another free HCAL
        std::multimap<double, unsigned> hcalElems;
        block.associatedElements(iHO, linkData, hcalElems,
                                 reco::PFBlockElement::HCAL,
                                 reco::PFBlock::LINKTEST_ALL);
        ...
        if (debug_ && useHO_) {
          cout << "\telement " << elements[iHO] << " linked with dist " << dist
               << endl;
          cout << "Added to HCAL cluster to form a neutral hadron" << endl;
        }
        ...
        double hoEnergy =
            hoclusterRef
                ->energy(); // calibration_->energyEm(*hoclusterRef,ps1Ene,ps2Ene,crackCorrection);
    ...
    // std::cout << "CalibEcal,HCal = " << calibEcal << ", " << calibHcal <<
    // std::endl; std::cout <<
    // "-------------------------------------------------------------------" <<
    // std::endl; ECAL energy : calibration
    ...
    auto &cand = (*pfCandidates_)[reconstructCluster(*hclusterRef,
                                                     calibEcal + calibHcal)];
    ...
    if (!useHO_) {
      ...
      cand.setHcalEnergy(max(totalHcal - totalHO, 0.0),
                         calibHcal * (1. - totalHO / totalHcal));
...
void PFAlgo::ECALLoop(const reco::PFBlock &block, reco::PFBlock::LinkData& linkData, const edm::OwnVector<reco::PFBlockElement> &elements, std::vector<bool>& active, const reco::PFBlockRef &blockref, ElementIndices& inds, std::vector<bool> &deadArea) {
  for (unsigned i = 0; i < inds.ecalIs.size(); i++) {
    ...
    auto &cand =
        (*pfCandidates_)[reconstructCluster(*clusterref, particleEnergy)];
...
void PFAlgo::processBlock(const reco::PFBlockRef &blockref,
  ...
  ElementIndices inds;
  EleLoop1(block, linkData, elements, active, blockref, inds, deadArea);
  ...
  HCALLoop(block, linkData, elements, active, blockref, inds, deadArea);
  ...
  HCALUnlinkedLoop(block, linkData, elements, active, blockref, inds, deadArea);
  ...
  ECALLoop(block, linkData, elements, active, blockref, inds, deadArea);
...
unsigned PFAlgo::reconstructTrack(const reco::PFBlockElement &elt,
  ...
  if (debug_)
    cout << "Reconstructing PF candidate from track of pT = " << track.pt()
  ...
  pfCandidates_->back().setPositionAtECALEntrance(
      eltTrack->positionAtECALEntrance());
  ...
  bool isMuon =
      pfmu_->reconstructMuon(pfCandidates_->back(), muonRef, allowLoose);
  bool isFromDisp = isFromSecInt(elt, "secondary");

  if ((!isMuon) && isFromDisp) {
    double Dpt = trackRef->ptError();
    double dptRel = Dpt / trackRef->pt() * 100;
    // If the track is ill measured it is better to not refit it, since the
    // track information probably would not be used. In the PFAlgo we use the
    // trackref information. If the track error is too big the refitted
    // information might be very different
    // from the not refitted one.
    if (dptRel < dptRel_DispVtx_) {
      if (debug_)
        cout << "Not refitted px = " << px << " py = " << py << " pz = " << pz
             << " energy = " << energy << endl;
      // reco::TrackRef trackRef = eltTrack->trackRef();
      reco::PFDisplacedVertexRef vRef =
          eltTrack->displacedVertexRef(reco::PFBlockElement::T_FROM_DISP)
              ->displacedVertexRef();
      ...
      ::math::XYZTLorentzVector momentum(
          trackRefit.px(), trackRefit.py(), trackRefit.pz(),
          sqrt(trackRefit.p() * trackRefit.p() + 0.13957 * 0.13957));
      if (debug_)
        cout << "Refitted px = " << px << " py = " << py << " pz = " << pz
             << " energy = " << energy << endl;
    }
  ...
  // do not label as primary a track which would be recognised as a muon. A muon
  // cannot produce NI. It is with high probability a fake
  if (isFromSecInt(elt, "primary") && !isMuon) {
    pfCandidates_->back().setFlag(reco::PFCandidate::T_TO_DISP, true);
    pfCandidates_->back().setDisplacedVertexRef(
        eltTrack->displacedVertexRef(reco::PFBlockElement::T_TO_DISP)
            ->displacedVertexRef(),
...
unsigned PFAlgo::reconstructCluster(const reco::PFCluster &cluster,
                                    double particleEnergy, bool useDirection,
                                    double particleX, double particleY,
                                    double particleZ) {
  ...
  if (useDirection) {
    ...
    case PFLayer::HCAL_BARREL1:
      factor = std::sqrt(cluster.position().Perp2() /
  ...
  if (useVertices_)
    vertexPos = ::math::XYZPoint(primaryVertex_.x(), primaryVertex_.y(),
                                 primaryVertex_.z());
  else
  ...
  //::math::XYZVector clusterPos( cluster.position().X(),
  //cluster.position().Y(),cluster.position().Z() );
  ...
  pfCandidates_->back().setPositionAtECALEntrance(::math::XYZPointF(
    ...
    if (DetId(hit.detId()).det() == DetId::Hcal) {
      ...
      if (hit.depth() < 1 || hit.depth() > 7) {
        throw cms::Exception("CorruptData")
            << "Bogus depth " << hit.depth() << " at detid " << hit.detId()
            << "\n";
      }
      energyPerDepth[hit.depth() - 1] +=
          hitRefAndFrac.fraction() * hit.energy();
    }
  }
  double sum =
      std::accumulate(energyPerDepth.begin(), energyPerDepth.end(), 0.);
...
ostream &operator<<(ostream &out, const PFAlgo &algo) {
  ...
  const std::unique_ptr<reco::PFCandidateCollection> &candidates =
      algo.pfCandidates();
...
void PFAlgo::associatePSClusters(
    unsigned iEcal, reco::PFBlockElement::Type psElementType,
    const reco::PFBlock &block,
    const edm::OwnVector<reco::PFBlockElement> &elements,
    const reco::PFBlock::LinkData &linkData, std::vector<bool> &active,
  ...
  block.associatedElements(iEcal, linkData, sortedPS, psElementType,
                           reco::PFBlock::LINKTEST_ALL);
  ...
  for (auto const &ps : sortedPS) {
    ...
    std::multimap<double, unsigned> sortedECAL;
    block.associatedElements(iPS, linkData, sortedECAL,
                             reco::PFBlockElement::ECAL,
                             reco::PFBlock::LINKTEST_ALL);
...
bool PFAlgo::isFromSecInt(const reco::PFBlockElement &eTrack,
  ...
  reco::PFBlockElement::TrackType T_FROM_DISP =
      reco::PFBlockElement::T_FROM_DISP;
  //  reco::PFBlockElement::TrackType T_FROM_GAMMACONV =
  ...
  //   bool isFromConv = usePFConversions_ &&
  //   eTrack.trackType(T_FROM_GAMMACONV);

  //   if ((bAll || bSecondary)&& isFromConv) return true;

  bool isFromDecay =
      (bAll || bSecondary) && usePFDecays_ && eTrack.trackType(T_FROM_V0);

  return isFromDecay;
}

void PFAlgo::setEGElectronCollection(
    const reco::GsfElectronCollection &egelectrons) {
...
void PFAlgo::postCleaning() {
  ...
  if (significance > minSignificance_) {
    ...
    while (next) {
      ...
      for (unsigned i = 0; i < pfCandidates_->size(); ++i) {
        ...
        if (pfc.particleId() != reco::PFCandidate::h_HF &&
            pfc.particleId() != reco::PFCandidate::egamma_HF)
          continue;
        ...
        const bool skip = std::any_of(pfCandidatesToBeRemoved.begin(),
                                      pfCandidatesToBeRemoved.end(),
                                      [&](unsigned int j) { return i == j; });
        if (skip)
          continue;

        // Check that the pt and the MET are aligned
        deltaPhi = std::acos((metX * pfc.px() + metY * pfc.py()) /
                             (pfc.pt() * std::sqrt(met2)));
      ...
      //
      // If the MET must be significanly reduced, schedule the candidate to be
      // cleaned
    ...
    //
    // The significance must be significantly reduced to indeed clean the
    // candidates
...
void PFAlgo::checkCleaning(const reco::PFRecHitCollection &cleanedHits) {
  ...
  if (std::sqrt(met2_Original) - std::sqrt(met2) > 5.) {
    ...
    for (unsigned int hitIdx : hitsToBeAdded) {
      ...
      PFCluster cluster(hit.layer(), hit.energy(), hit.position().x(),
                        hit.position().y(), hit.position().z());
      reconstructCluster(cluster, hit.energy());
      if (debug_) {
        std::cout << pfCandidates_->back() << ". time = " << hit.time()
                  << std::endl;
      }
    }
  }
...
void PFAlgo::setElectronExtraRef(
    const edm::OrphanHandle<reco::PFCandidateElectronExtraCollection> &extrah) {
  if (!usePFElectrons_)
    return;
  ...
  for (auto &cand : *pfCandidates_) {
    ...
    if (cand.particleId() == PFCandidate::e) {
      ...
      std::vector<PFCandidateElectronExtra>::const_iterator it = find_if(
          pfElectronExtra_.begin(), pfElectronExtra_.end(), myExtraEqual);
      if (it != pfElectronExtra_.end()) {
        //	std::cout << " Index " << it-pfElectronExtra_.begin() <<
        //std::endl;
        reco::PFCandidateElectronExtraRef theRef(extrah,
                                                 it - pfElectronExtra_.begin());
    ...
    } else // else save the mva and the extra as well !
    {
      if (cand.trackRef().isNonnull()) {
        ...
        if (it != pfElectronExtra_.end()) {
          ...
          reco::PFCandidateElectronExtraRef theRef(
              extrah, it - pfElectronExtra_.begin());
          cand.setPFElectronExtraRef(theRef);
  ...
  for (auto &ele : *pfElectronCandidates_) {
    // select the electrons - this test is actually not needed for this
    // collection
...
void PFAlgo::setPhotonExtraRef(
  ...
  for (auto &cand : *pfCandidates_) {
    // select the electrons and add the extra
    if (cand.particleId() == PFCandidate::gamma &&
        cand.mva_nothing_gamma() > 0.99) {
