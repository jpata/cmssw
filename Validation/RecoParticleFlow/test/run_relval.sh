...
if [ $STEP == "RECO" ]; then
	...
	cmsDriver.py step3 --runUnscheduled  --conditions $CONDITIONS -s RAW2DIGI,L1Reco,RECO,RECOSIM,EI,PAT \
          --datatier MINIAODSIM --nThreads $NTHREADS -n -1 --era $ERA \
          --eventcontent MINIAODSIM --geometry=$GEOM --filein $FILENAME \
          --customise RecoParticleFlow/PFProducer/mlpfproducer_customize.customize_step3 \
          --fileout file:step3_inMINIAODSIM.root | tee step3.log  2>&1
